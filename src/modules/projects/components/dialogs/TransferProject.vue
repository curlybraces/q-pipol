<template>
	<q-card style="width:400px;">
		...
		<div class="q-pa-sm">
			<q-input outlined placeholder="Filter Users" v-model="filterUsers"></q-input>
		</div>	
		<q-option-group
			v-model="user_id"
		...
		<q-card-actions align="right">
			...
			<q-btn color="primary" label="Confirm" @click="confirmTransferProject" :disable="!user_id"></q-btn>
		</q-card-actions>
	</q-card>
</template>
			...
			encoders: {
				...
				result({ data }) {
					this.options = data.encoders.map(encoder => {
						...
						return {
							value: id,
							label: `${name} - ${operating_unit ? operating_unit.acronym: 'N/A'}`
						}
					})
				}
			}
			...
			confirmTransferProject() {
				if (this.user_id) {
					this.$q.dialog({
						...
						message: 'You will no longer be able to access project upon transfer. Continue?',
						cancel: true
					})
