<template>
  <q-card>
    ...
    <div class="row q-pa-sm q-col-gutter-md bg-white">
      <template v-for="(avatar,index) in AVATARS">
        <div class="col-3" :key="index">
          <q-img :src="`statics/avatar/${avatar}.svg`" @click="select(avatar)" class="cursor-pointer" :class="(selectedAvatar == avatar) ? 'active': void 0" />
    ...
    <q-card-actions align="right">
      ...
      <q-btn dense color="primary" label="Save" @click="saveAvatar" :loading="loading" />
    </q-card-actions>
  </q-card>
</template>
