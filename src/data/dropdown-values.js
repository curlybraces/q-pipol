export const APPROVAL_LEVELS = [
  {
    label: 'Yet to be submitted to the NEDA Secretariat',
    value: 'Yet to be submitted to the NEDA Secretariat'
  },
  {
    label: 'Under the NEDA Secretariat Review',
    value: 'Under the NEDA Secretariat Review'
  },
  {
    label: 'ICC-TB Endorsed',
    value: 'ICC-TB Endorsed'
  },
  {
    label: 'ICC-CC Approved',
    value: 'ICC-CC Approved'
  },
  {
    label: 'NEDA Board Confirmed',
    value: 'NEDA Board Confirmed'
  },
  {
    label: 'Not Applicable',
    value: 'Not Applicable'
  }
];

export const TIERS = [
  {
    id: '1',
    name: 'Tier 1'
  },
  {
    id: '2',
    name: 'Tier 2 (New)'
  },
  {
    id: '3',
    name: 'Tier 2 (Expanded/Revised)'
  }
];

export const CHALLENGES = [
  {
    value: 'Low farm productivity',
    label: 'Low farm productivity'
  },
  {
    value: 'Lack of labor',
    label: 'Lack of labor'
  },
  {
    value: 'Unaffordable and inaccessible credit',
    label: 'Unaffordable and inaccessible credit'
  },
  {
    value: 'Limited use of technology',
    label: 'Limited use of technology'
  },
  {
    value: 'Limited farmland diversification',
    label: 'Limited farmland diversification'
  },
  {
    value: 'Undeveloped agri-manufacturing and export',
    label: 'Undeveloped agri-manufacturing and export'
  },
  {
    value: 'Severe deforestation/land degradation',
    label: 'Severe deforestation/land degradation'
  },
  {
    value: 'Aging farmers and fisherfolk',
    label: 'Aging farmers and fisherfolk'
  },
  {
    value: 'Climate change',
    label: 'Climate change'
  }
];

export const FUNDING_INSTITUTIONS = [
  {
    label: 'Australia',
    value: 'Australia'
  },
  {
    label: 'Austria',
    value: 'Austria'
  },
  {
    label: 'China',
    value: 'China'
  },
  {
    label: 'Japan',
    value: 'Japan'
  },
  {
    label: 'Japan International Cooperation Agency ',
    value: 'Japan International Cooperation Agency '
  },
  {
    label: 'Korea',
    value: 'Korea'
  },
  {
    label: 'Korea International Cooperation Agency ',
    value: 'Korea International Cooperation Agency '
  },
  {
    label:
      'Korea Eximbank Resident Mission / Economic Development Cooperation Fund',
    value:
      'Korea Eximbank Resident Mission / Economic Development Cooperation Fund'
  },
  {
    label: 'New Zealand',
    value: 'New Zealand'
  },
  {
    label: 'Saudi Arabia',
    value: 'Saudi Arabia'
  },
  {
    label: 'Saudi Fund for Development',
    value: 'Saudi Fund for Development'
  },
  {
    label: 'Belgium',
    value: 'Belgium'
  },
  {
    label: 'Canada',
    value: 'Canada'
  },
  {
    label: 'Denmark',
    value: 'Denmark'
  },
  {
    label: 'European Union',
    value: 'European Union'
  },
  {
    label: 'European Investment Bank',
    value: 'European Investment Bank'
  },
  {
    label: 'France',
    value: 'France'
  },
  {
    label: 'French Development Agency Philippines',
    value: 'French Development Agency Philippines'
  },
  {
    label: 'Germary',
    value: 'Germary'
  },
  {
    label: 'Germany Agency for International Cooperation',
    value: 'Germany Agency for International Cooperation'
  },
  {
    label: 'German Development Bank',
    value: 'German Development Bank'
  },
  {
    label: 'Federal Ministry for Economic Cooperation and Development',
    value: 'Federal Ministry for Economic Cooperation and Development'
  },
  {
    label: 'Hungary',
    value: 'Hungary'
  },
  {
    label: 'Italy',
    value: 'Italy'
  },
  {
    label: 'Netherlands',
    value: 'Netherlands'
  },
  {
    label: 'Norway',
    value: 'Norway'
  },
  {
    label: 'Nordic Investment Bank',
    value: 'Nordic Investment Bank'
  },
  {
    label: 'Poland',
    value: 'Poland'
  },
  {
    label: 'Spain',
    value: 'Spain'
  },
  {
    label: 'Agencia Espanola de Cooperation International para el Desarollo',
    value: 'Agencia Espanola de Cooperation International para el Desarollo'
  },
  {
    label: 'Sweden/Swedish International Development Cooperation Agency',
    value: 'Sweden/Swedish International Development Cooperation Agency'
  },
  {
    label: 'Switzerland',
    value: 'Switzerland'
  },
  {
    label: 'United Kingdom',
    value: 'United Kingdom'
  },
  {
    label: 'United States',
    value: 'United States'
  },
  {
    label: 'United States Agency for International Development',
    value: 'United States Agency for International Development'
  },
  {
    label: 'Asian Development Bank',
    value: 'Asian Development Bank'
  },
  {
    label: 'World Bank',
    value: 'World Bank'
  },
  {
    label: 'United Nations Coordination Office',
    value: 'United Nations Coordination Office'
  },
  {
    label: 'Food and Agriculture Organization',
    value: 'Food and Agriculture Organization'
  },
  {
    label: 'United Nations Development Programme',
    value: 'United Nations Development Programme'
  },
  {
    label: 'United Nations Industrial Development Organization',
    value: 'United Nations Industrial Development Organization'
  },
  {
    label: 'United Nations Population Fund',
    value: 'United Nations Population Fund'
  },
  {
    label: 'International Fund for Agricultural Development',
    value: 'International Fund for Agricultural Development'
  },
  {
    label: 'International Labor Organization',
    value: 'International Labor Organization'
  },
  {
    label: "United Nations Children's Fund",
    value: "United Nations Children's Fund"
  },
  {
    label: 'International Atomic Energy Agency',
    value: 'International Atomic Energy Agency'
  },
  {
    label: 'OPEC Fund for International Development',
    value: 'OPEC Fund for International Development'
  },
  {
    label: 'World Food Programme',
    value: 'World Food Programme'
  },
  {
    label: 'World Health Organization',
    value: 'World Health Organization'
  },
  {
    label: 'To be detemined',
    value: 'To be detemined'
  },
  {
    label: 'Others',
    value: 'Others'
  }
];

export const CIP_TYPES = [
  {
    label:
      'Locally-funded major capital programs/projects with total project cost of at least PhP2.5 billion',
    value:
      'Locally-funded major capital programs/projects with total project cost of at least PhP2.5 billion'
  },
  {
    label:
      'ODA-assisted grants with total project cost of at least PhP2.5 billion',
    value:
      'ODA-assisted grants with total project cost of at least PhP2.5 billion'
  },
  {
    label:
      'ODA-assisted loans regardless of amount that requires national government guarantee',
    value:
      'ODA-assisted loans regardless of amount that requires national government guarantee'
  },
  {
    label:
      'Relending activities to LGUs and other target beneficiaries with total project cost of at least PhP 2.5 billion',
    value:
      'Relending activities to LGUs and other target beneficiaries with total project cost of at least PhP 2.5 billion'
  },
  {
    label: 'Solicited national PPP projects',
    value: 'Solicited national PPP projects'
  },
  {
    label:
      'Joint Venture (JV) Agreement with with government contribution amounting of at least PhP150 Million',
    value:
      'Joint Venture (JV) Agreement with with government contribution amounting of at least PhP150 Million'
  },
  {
    label:
      'Administrative buildings with total project cost of at least PhP1 billion',
    value:
      'Administrative buildings with total project cost of at least PhP1 billion'
  },
  {
    label: 'Not Applicable',
    value: 'Not Applicable'
  }
];

export const FUNDING_SOURCES = [
  {
    label: 'NG-Local',
    value: 1
  },
  {
    label: 'NG-ODA Loan',
    value: 2
  },
  {
    label: 'NG-ODA Grant',
    value: 3
  },
  {
    label: 'GOCC/GFIs',
    value: 4
  },
  {
    label: 'LGUs',
    value: 5
  },
  {
    label: 'Private Sector',
    value: 6
  },
  {
    label: 'Others',
    value: 7
  }
];

export const INFRASTRUCTURE_SECTORS = [
  {
    label: 'Social Infrastructure',
    value: 'Social Infrastructure',
    children: [
      {
        label: 'Health',
        value: 'Health'
      },
      {
        label: 'Education',
        value: 'Education'
      },
      {
        label: 'Solid Waste Management',
        value: 'Solid Waste Management'
      },
      {
        label: 'Housing',
        value: 'Housing'
      },
      {
        label: 'Public Safety/Security',
        value: 'Public Safety/Security'
      },
      {
        label: 'Urban/Heritage Renewal',
        value: 'Urban/Heritage Renewal'
      }
    ]
  },
  {
    label: 'Power - Electrification',
    value: 'Power - Electrification',
    children: []
  },
  {
    label: 'Transportation',
    value: 'Transportation',
    children: [
      {
        label: 'Roads and Bridges',
        value: 'Roads and Bridges'
      },
      {
        label: 'Water Transportation',
        value: 'Water Transportation'
      },
      {
        label: 'Air Transportation',
        value: 'Air Transportation'
      },
      {
        label: 'Rail Transportation',
        value: 'Rail Transportation'
      },
      {
        label: 'Urban Transportation',
        value: 'Urban Transportation'
      }
    ]
  },
  {
    label: 'Water Resources',
    value: 'Water Resources',
    children: [
      {
        label: 'Irrigation',
        value: 'Irrigation'
      },
      {
        label: 'Water Supply',
        value: 'Water Supply'
      },
      {
        label: 'Flood Management',
        value: 'Flood Management'
      },
      {
        label: 'Sanitation/Sewerage/Septage',
        value: 'Sanitation/Sewerage/Septage'
      }
    ]
  },
  {
    label: 'Information and Communications Technology',
    value: 'Information and Communications Technology',
    children: []
  },
  {
    label: 'Others',
    value: 'Others',
    children: [
      {
        label: 'Reclamation',
        value: 'Reclamation'
      },
      {
        label: 'Government Building',
        value: 'Government Building'
      },
      {
        label: 'Multipurpose Facilities',
        value: 'Multipurpose Facilities'
      },
      {
        label: 'Others',
        value: 'Others'
      }
    ]
  }
];

export const GAD_RESPONSIVENESS = [
  {
    label: 'GAD is invisible',
    value: 'GAD is invisible'
  },
  {
    label: 'Program/Project has promising GAD prospects',
    value: 'Program/Project has promising GAD prospects'
  },
  {
    label: 'Program/Project is gender-sensitive',
    value: 'Program/Project is gender-sensitive'
  },
  {
    label: 'Program/Project is gender-responsive',
    value: 'Program/Project is gender-responsive'
  },
  {
    label: 'Not Applicable',
    value: 'Not Applicable'
  }
];

export const IMPLEMENTATION_READINESS = [
  {
    label: 'Pre-Feasibility Study/Business Case',
    value: 'Pre-Feasibility Study/Business Case'
  },
  {
    label: 'Feasibility Study',
    value: 'Feasibility Study'
  },
  {
    label: 'Level of Approval',
    value: 'Level of Approval'
  },
  {
    label: 'Right-of-Way Acquisition',
    value: 'Right-of-Way Acquisition'
  },
  {
    label: 'Resettlement Action Plan',
    value: 'Resettlement Action Plan'
  },
  {
    label: 'Environmental Compliance Certificate',
    value: 'Environmental Compliance Certificate'
  },
  {
    label: 'RDC Endorsement',
    value: 'RDC Endorsement'
  },
  {
    label: 'Detailed Engineering Design',
    value: 'Detailed Engineering Design'
  },
  {
    label: 'Other Pre-Investment Activities',
    value: 'Other Pre-Investment Activities'
  }
];

export const IMPLEMENTATION_BASES = [
  {
    id: '1',
    name: 'National Expenditure Program (NEP)'
  },
  {
    id: '2',
    name: 'General Appropriations Act (GAA)'
  },
  {
    id: '3',
    name: 'Multi-Year Obligational Authority (MYOA)'
  },
  {
    id: '4',
    name: 'Existing masterplan/sector studies/procurement plan'
  },
  {
    id: '5',
    name: 'List of Regional Development Council-endorsed projects'
  },
  {
    id: '6',
    name: 'Agreements (e.g. Peace agreements)'
  },
  {
    id: '7',
    name: 'Existing laws, rules or regulations'
  }
];

export const IMPLEMENTATION_MODES = [
  {
    id: '1',
    name:
      'Through Local Funds in accordance with RA 9184 or the Government Procurement Act'
  },
  {
    id: '2',
    name: 'Through ODA pursuant to RA 8182 or the ODA Act of 1996'
  },
  {
    id: '3',
    name: 'Through PPP under the Amended BOT Law and Its IRR'
  },
  {
    id: '4',
    name: 'Through Joint Venture Arrangement'
  },
  {
    id: '5',
    name: 'Others'
  }
];

export const NEDA_PROCESSING = [
  {
    label: 'Yet to be submitted to NEDA Secretariat ',
    value: 'Yet to be submitted to NEDA Secretariat '
  },
  {
    label: 'Under the NEDA Secretariat Review',
    value: 'Under the NEDA Secretariat Review'
  },
  {
    label: 'ICC-B Endorsed',
    value: 'ICC-B Endorsed'
  },
  {
    label: 'ICC-CC Approved',
    value: 'ICC-CC Approved'
  },
  {
    label: 'NEDA Board Confirmed',
    value: 'NEDA Board Confirmed'
  },
  {
    label: 'Not Applicable',
    value: 'Not Applicable'
  }
];

export const NEW_THINKINGS = [
  {
    value: 'Modernization of Agriculture',
    label: '1. Modernization of Agriculture'
  },
  {
    value: 'Industrialization of Agriculture',
    label: '2. Industrialization of Agriculture'
  },
  {
    value: 'Promotion of Exports',
    label: '3. Promotion of Exports'
  },
  {
    value: 'Farm Consolidation',
    label: '4. Farm Consolidation'
  },
  {
    value: 'Roadmap Development',
    label: '5. Roadmap Development'
  },
  {
    value: 'Infrastructure Development',
    label: '6. Infrastructure Development'
  },
  {
    value: 'Higher Budget and Investments for Agriculture',
    label: '7. Higher Budget and Investments for Agriculture'
  },
  {
    value: 'Legislative Support',
    label: '8. Legislative Support'
  }
];

export const TYPES = [
  {
    name: 'Program',
    id: '1'
  },
  {
    name: 'Project',
    id: '2'
  }
];

export const PIP_TYPES = [
  {
    label: 'Capital Investment Program and Project',
    value: 'Capital Investment Program and Project'
  },
  {
    label:
      'Technical Assistance Program/Project (such as Research and Development, Institutional Development, Human Resource Capacity Building, or System/Process Improvement PAPs)',
    value:
      'Technical Assistance Program/Project (such as Research and Development, Institutional Development, Human Resource Capacity Building, or System/Process Improvement PAPs)'
  },
  {
    label: 'Relending Program/Project of GFIs to LGUs and Targets Beneficiary',
    value: 'Relending Program/Project of GFIs to LGUs and Targets Beneficiary'
  },
  {
    label: 'Government Buildings',
    value: 'Government Buildings'
  }
];

export const READINESS_LEVELS = [
  {
    label: 'Level 1 CIP/Non-CIP',
    value: 'Level 1 CIP/Non-CIP'
  },
  {
    label: 'Level 2 CIP/Non-CIP',
    value: 'Level 2 CIP/Non-CIP'
  },
  {
    label: 'Level 3 CIP/Non-CIP',
    value: 'Level 3 CIP/Non-CIP'
  },
  {
    label: 'Level 4 CIP/Non-CIP',
    value: 'Level 4 CIP/Non-CIP'
  }
];

export const REGIONS = [
  // {
  //   label: "Nationwide",
  //   order: 1,
  //   value: 999
  // },
  // {
  //   label: "Inter-Regional",
  //   order: 2,
  //   value: 998
  // },
  {
    label: 'NCR',
    order: 3,
    value: 13
  },
  {
    label: 'CAR',
    order: 4,
    value: 14
  },
  {
    label: 'Region I',
    order: 5,
    value: 1
  },
  {
    label: 'Region II',
    order: 6,
    value: 2
  },
  {
    label: 'Region III',
    order: 7,
    value: 3
  },
  {
    label: 'Region IV-A',
    order: 8,
    value: 4
  },
  {
    label: 'Region IV-B',
    order: 9,
    value: 17
  },
  {
    label: 'Region V',
    order: 10,
    value: 5
  },
  {
    label: 'Region VI',
    order: 11,
    value: 6
  },
  {
    label: 'Region VII',
    order: 12,
    value: 7
  },
  {
    label: 'Region VIII',
    order: 13,
    value: 8
  },
  {
    label: 'Region IX',
    order: 14,
    value: 9
  },
  {
    label: 'Region X',
    order: 15,
    value: 10
  },
  {
    label: 'Region XI',
    order: 16,
    value: 11
  },
  {
    label: 'Region XII',
    order: 17,
    value: 12
  },
  {
    label: 'Region XIII',
    order: 18,
    value: 16
  },
  {
    label: 'BARMM',
    order: 19,
    value: 15
  }
];

export const TYPOLOGIES = [
  {
    id: '1',
    name: 'Capital Investment Programs/Projects'
  },
  {
    id: '2',
    name: 'Technical Assistance'
  },
  {
    id: '3',
    name: 'Relending PAPs'
  },
  {
    id: '4',
    name: 'Government Building'
  }
];

export const TEN_POINT_AGENDA = [
  {
    label: '0. Peace and Order',
    value: 'Peace and Order'
  },
  {
    label:
      '1. Continue and maintain the current macroeconomic policies, including fiscal, monetary and trade policy',
    value:
      'Continue and maintain the current macroeconomic policies, including fiscal, monetary and trade policy'
  },
  {
    label:
      '2. Institute progressive tax reform and more effective tax collection, indexing taxes to inflation. A tax reform package will be submitted to Congress by September 2016.',
    value:
      'Institute progressive tax reform and more effective tax collection, indexing taxes to inflation. A tax reform package will be submitted to Congress by September 2016.'
  },
  {
    label:
      '3. Increase competitiveness and the ease of doing business. This effort will draw upon successful models used to attract business to local cities (e.g., Davao), and pursue the relaxation of the Constitutional restrictions on foreign ownership, except as regards land ownership, in order to attract foreign investments',
    value:
      'Increase competitiveness and the ease of doing business. This effort will draw upon successful models used to attract business to local cities (e.g., Davao), and pursue the relaxation of the Constitutional restrictions on foreign ownership, except as regards land ownership, in order to attract foreign investments'
  },
  {
    label:
      '4. Accelerate annual infrastructure spending to account for 5% of GDP, with Public-Private Partnerships playing a key role',
    value:
      'Accelerate annual infrastructure spending to account for 5% of GDP, with Public-Private Partnerships playing a key role'
  },
  {
    label:
      '5. Promote rural and value chain development toward increasing agricultural and rural enterprise productivity and rural tourism.',
    value:
      'Promote rural and value chain development toward increasing agricultural and rural enterprise productivity and rural tourism.'
  },
  {
    label:
      '6. Ensure security of land tenure to encourage investments, and address bottlenecks in land management and titling agencies.',
    value:
      'Ensure security of land tenure to encourage investments, and address bottlenecks in land management and titling agencies.'
  },
  {
    label:
      '7. Invest in human capital development, including health and education systems, and match skills and training to meet the demand of businesses and the private sector.',
    value:
      'Invest in human capital development, including health and education systems, and match skills and training to meet the demand of businesses and the private sector.'
  },
  {
    label:
      '8. Promote science, technology, and the creative arts to enhance innovation and creative capacity towards self-sustaining, inclusive development.',
    value:
      'Promote science, technology, and the creative arts to enhance innovation and creative capacity towards self-sustaining, inclusive development.'
  },
  {
    label:
      "9. Improve social protection programs, including the government's Conditional Cash Transfer program, to protect the poor against instability and economic shocks.",
    value:
      "Improve social protection programs, including the government's Conditional Cash Transfer program, to protect the poor against instability and economic shocks."
  },
  {
    label:
      '10. Strengthen implementation of the Responsible Parenthood and Reproductive Health Law to enable especially poor couples to make informed choices on financial and family planning.',
    value:
      'Strengthen implementation of the Responsible Parenthood and Reproductive Health Law to enable especially poor couples to make informed choices on financial and family planning.'
  }
];

export const SUSTAINABLE_DEVELOPMENT_GOALS = [
  {
    value: '1',
    label: 'No Poverty'
  },
  {
    value: '2',
    label: 'Zero Hunger'
  },
  {
    value: '3',
    label: 'Good Health and Well-Being'
  },
  {
    value: '4',
    label: 'Quality Education'
  },
  {
    value: '5',
    label: 'Gender Equality'
  },
  {
    value: '6',
    label: 'Clean Water and Sanitation'
  },
  {
    value: '7',
    label: 'Affordable and Clean Energy'
  },
  {
    value: '8',
    label: 'Decent Work and Economic Growth'
  },
  {
    value: '9',
    label: 'Industry, Innovation and Infrastructure'
  },
  {
    value: '10',
    label: 'Reduced Inequalities'
  },
  {
    value: '11',
    label: 'Sustainable Cities and Communities'
  },
  {
    value: '12',
    label: 'Responsible Consumption and Production'
  },
  {
    value: '13',
    label: 'Climate Action'
  },
  {
    value: '14',
    label: 'Life Below Water'
  },
  {
    value: '15',
    label: 'Life on Land'
  },
  {
    value: '16',
    label: 'Peace, Justice and Strong Institutions'
  },
  {
    value: '17',
    label: 'Partnerships for the Goals'
  }
];

export const PRIORITY_CONCERNS = [
  {
    value:
      '1. Ensure the proper and efficient implementation of the Rice Tariffication Law, particularly the management of the P10-B Rice Competitiveness Enhancement Fund (RCEF) including that of the formulation of the Rice Industry Roadmap.',
    label:
      '1. Ensure the proper and efficient implementation of the Rice Tariffication Law, particularly the management of the P10-B Rice Competitiveness Enhancement Fund (RCEF) including that of the formulation of the Rice Industry Roadmap.'
  },
  {
    value:
      '2. Craft and implement a “Crop Diversification Strategy” with corresponding fund requirements, initially using current DA program budget, and thereafter included under the 2020 and 2021-2022 annual budgets.',
    label:
      '2. Craft and implement a “Crop Diversification Strategy” with corresponding fund requirements, initially using current DA program budget, and thereafter included under the 2020 and 2021-2022 annual budgets.'
  },
  {
    value:
      '3. Craft and implement a “Pantawid Magsasaka Program” that will provide continuous assistance to adversely affected rice farmers due to the rice tariffication regime.',
    label:
      '3. Craft and implement a “Pantawid Magsasaka Program” that will provide continuous assistance to adversely affected rice farmers due to the rice tariffication regime.'
  },
  {
    value:
      '4. Enhance skills of small farmers and fisherfolk, with emphasis on entrepreneurship. This includes empowering small farmers’ and fisherfolk’s cooperatives (SFFCs), irrigators’ associations (IAs) and agrarian reform beneficiaries’ associations (ARBAs). They should be trained on entrepreneurship and financial management. DA should forge MOAs with major agribusiness companies that will provide business managers who will train SFFCs, IAs and ARBAs on how to profitably run their farm fishery and agribusiness enterprises.',
    label:
      '4. Enhance skills of small farmers and fisherfolk, with emphasis on entrepreneurship. This includes empowering small farmers’ and fisherfolk’s cooperatives (SFFCs), irrigators’ associations (IAs) and agrarian reform beneficiaries’ associations (ARBAs). They should be trained on entrepreneurship and financial management. DA should forge MOAs with major agribusiness companies that will provide business managers who will train SFFCs, IAs and ARBAs on how to profitably run their farm fishery and agribusiness enterprises.'
  },
  {
    value:
      '5. Strongly implement climate change and disaster risk reduction program. This includes\ninstitutionalizing early warning systems and protocols, such as typhoon and flood\nreadiness, harvesting of crops using combine harvesters and storing them in all-\nweather, earthquake-proof and elevated sturdy warehouses.',
    label:
      '5. Strongly implement climate change and disaster risk reduction program. This includes\ninstitutionalizing early warning systems and protocols, such as typhoon and flood\nreadiness, harvesting of crops using combine harvesters and storing them in all-\nweather, earthquake-proof and elevated sturdy warehouses.'
  },
  {
    value:
      '6. Assist BARRM in implementing its agriculture, fishery and agri-entrepreneurship\ndevelopment program.',
    label:
      '6. Assist BARRM in implementing its agriculture, fishery and agri-entrepreneurship\ndevelopment program.'
  },
  {
    value:
      '7. Introduce “New Thinking” as the revised DA vision and mission, like strive to attain: a\n“food-secure” Philippines teeming with prosperous farmers and fisherfolk; identify and\npromote “countryside heroes” that include successful farmers, fisherfolk, farmers’ and\nfishers’ groups; establish and strengthen a farmers’ and /fisherfolk’s registry to serve as\nbasis for granting social benefits like SSS, Philhealth, etc.',
    label:
      '7. Introduce “New Thinking” as the revised DA vision and mission, like strive to attain: a\n“food-secure” Philippines teeming with prosperous farmers and fisherfolk; identify and\npromote “countryside heroes” that include successful farmers, fisherfolk, farmers’ and\nfishers’ groups; establish and strengthen a farmers’ and /fisherfolk’s registry to serve as\nbasis for granting social benefits like SSS, Philhealth, etc.'
  },
  {
    value:
      '8. Sustain massive information and communication campaign, highlighting countryside\nheroes.',
    label:
      '8. Sustain massive information and communication campaign, highlighting countryside\nheroes.'
  },
  {
    value: 9,
    label:
      '9. Review and re-orient existing DA programs and projects towards increasing productivity,\ncompetitiveness and income.'
  },
  {
    value: '10. Reshape and reprogram budget of DA for 2020.',
    label: '10. Reshape and reprogram budget of DA for 2020.'
  },
  {
    value:
      '11. Enhance partnership with the private sector particularly in forging contract-growing and\nmarketing agreements, and training of SFFCs, IAs, and ARBAs on agribusiness,\nentrepreneurship, financial and marketing.\nSuch private groups include the Go Negosyo of PA Joey Concepcion, Philippine Chamber of\nCommerce and Industry, the Fil-Chinese CCI (chaired by Henry Lim), Management Assn of the\nPhils. (MAP), and multinational agribusiness companies (SMC, JG Summit, MVP Group of\nCompanies, etc.).',
    label:
      '11. Enhance partnership with the private sector particularly in forging contract-growing and\nmarketing agreements, and training of SFFCs, IAs, and ARBAs on agribusiness,\nentrepreneurship, financial and marketing.\nSuch private groups include the Go Negosyo of PA Joey Concepcion, Philippine Chamber of\nCommerce and Industry, the Fil-Chinese CCI (chaired by Henry Lim), Management Assn of the\nPhils. (MAP), and multinational agribusiness companies (SMC, JG Summit, MVP Group of\nCompanies, etc.).'
  },
  {
    value:
      '12. Strengthen the organizational structure of DA including systems and processes for\nefficient and effective governance.\nThis includes energizing the provincial and municipal agriculture offices (PAOs and MAOs) and\ntheir technicians.',
    label:
      '12. Strengthen the organizational structure of DA including systems and processes for\nefficient and effective governance.\nThis includes energizing the provincial and municipal agriculture offices (PAOs and MAOs) and\ntheir technicians.'
  },
  {
    value:
      '13. Conduct “Food Summit” with key stakeholders: farmers, fisherfolk, GOs, NGOs, LGUs,\nprivate sector, SUCs, others.\nThis includes strengthening the SUCs, mobilizing them to train farmers, fishers and rural\nentrepreneurs, as well as for lening technical assistance in extension work in close coordination\nwith DA Regional Offices and the provincial LGUs.\nDA should provide SUCs budget for such activities.',
    label:
      '13. Conduct “Food Summit” with key stakeholders: farmers, fisherfolk, GOs, NGOs, LGUs,\nprivate sector, SUCs, others.\nThis includes strengthening the SUCs, mobilizing them to train farmers, fishers and rural\nentrepreneurs, as well as for lending technical assistance in extension work in close coordination\nwith DA Regional Offices and the provincial LGUs.\nDA should provide SUCs budget for such activities.'
  },
  {
    value:
      '14. Coordinate/collaborate with DTI regarding price stabilization and complementation of\nagricultural products as well as strengthening quarantine measures against poultry and\nlivestock diseases like the African swine fever.',
    label:
      '14. Coordinate/collaborate with DTI regarding price stabilization and complementation of\nagricultural products as well as strengthening quarantine measures against poultry and\nlivestock diseases like the African swine fever.'
  },
  {
    value:
      '15. Review restrictive and constrictive policies on agriculture, fishery, agribusiness, credit,\netc. For instance, the possibility of lifting the ban on export of mature coconuts and\ncorn.',
    label:
      '15. Review restrictive and constrictive policies on agriculture, fishery, agribusiness, credit,\netc. For instance, the possibility of lifting the ban on export of mature coconuts and\ncorn.'
  }
];

export const PREREQUISITES = [
  {
    label: 'NEDA Board',
    value: 'NEDA Board'
  },
  {
    label: 'NEDA Board - ICC',
    value: 'NEDA Board - ICC'
  },
  {
    label: 'DPWH Certification',
    value: 'DPWH Certification'
  },
  {
    label: 'DPWH MOA',
    value: 'DPWH MOA'
  },
  {
    label: 'DPWH Costing',
    value: 'DPWH Costing'
  },
  {
    label: 'DENR Clearance',
    value: 'DENR Clearance'
  },
  {
    label: 'RDC Consultation',
    value: 'RDC Consultation'
  },
  {
    label: 'CSO Consultation',
    value: 'CSO Consultation'
  },
  {
    label: 'List of Locations',
    value: 'List of Locations'
  },
  {
    label: 'List of Beneficiaries',
    value: 'List of Beneficiaries'
  },
  {
    label: 'Evaluated and Endorsed by the DA Clearing House',
    value: 'Evaluated and Endorsed by the DA Clearing House'
  }
];

export const PREPARATION_DOCUMENTS = [
  {
    label: 'Concept Note',
    value: 'Concept Note'
  },
  {
    label: 'Project Proposal',
    value: 'Project Proposal'
  },
  {
    label: 'Feasibility Study',
    value: 'Feasibility Study'
  },
  {
    label: 'Detailed Engineering Design',
    value: 'Detailed Engineering Design'
  },
  {
    label: 'Business Plan/Case',
    value: 'Business Plan/Case'
  },
  {
    label: 'Right of Way',
    value: 'Right of Way'
  },
  {
    label: 'Resettlement Plan',
    value: 'Resettlement Plan'
  },
  {
    label: 'Others',
    value: 'Others'
  }
];

export const PROGRAMS = [
  {
    value: 'Production Support Services',
    label: 'PSS'
  },
  {
    value: 'Market Development Services',
    label: 'MDS'
  },
  {
    value: 'Research and Development',
    label: 'R&D'
  },
  {
    value: 'Extension Support, Education and Training Services',
    label: 'ESETS'
  },
  {
    value:
      'Agricultural Machineries, Equipment and Facilities Support Services',
    label: 'AMEFSS'
  },
  {
    value: 'Farm-to-Market Road Network Services',
    label: 'FMRDP'
  },
  {
    value: 'Irrigation Network Services',
    label: 'INS'
  },
  {
    value: 'Locally and Foreign Assisted Projects',
    label: 'LFP/FAP'
  },
  {
    value: 'Agriculture and Fisheries Regulatory Program',
    label: 'REGULATORY'
  },
  {
    value: 'Agriculture and Fisheries Policy Services',
    label: 'POLICY'
  },
  {
    value: 'Others',
    label: 'OTHERS'
  }
];

export const COMMODITY_GROUP = [
  {
    value: 'Rice',
    label: 'Rice'
  },
  {
    value: 'Corn',
    label: 'Corn'
  },
  {
    value: 'High Value Crops',
    label: 'High Value Crops'
  },
  {
    value: 'Livestock',
    label: 'Livestock'
  },
  {
    value: 'Fisheries',
    label: 'Fisheries'
  },
  {
    value: 'Organic',
    label: 'Organic'
  },
  {
    value: 'Halal',
    label: 'Halal'
  },
  {
    value: 'Not Applicable',
    label: 'Not Applicable'
  },
  {
    value: 'Commodity-Neutral',
    label: 'Commodity-Neutral'
  }
];

export const OPERATING_UNITS = [
  {
    value: 1,
    name: 'Central Office',
    label: 'DA-CO'
  },
  {
    value: 2,
    name: 'Regional Field Office - CAR',
    label: 'DA-RFO-CAR'
  },
  {
    value: 3,
    name: 'Regional Field Office - I',
    label: 'DA-RFO-I'
  },
  {
    value: 4,
    name: 'Regional Field Office - II',
    label: 'DA-RFO-II'
  },
  {
    value: 5,
    name: 'Regional Field Office - III',
    label: 'DA-RFO-III'
  },
  {
    value: 6,
    name: 'Regional Field Office - IVA',
    label: 'DA-RFO-IVA'
  },
  {
    value: 7,
    name: 'Regional Field Office - IVB',
    label: 'DA-RFO-IVB'
  },
  {
    value: 8,
    name: 'Regional Field Office - IX',
    label: 'DA-RFO-IX'
  },
  {
    value: 9,
    name: 'Regional Field Office - V',
    label: 'DA-RFO-V'
  },
  {
    value: 10,
    name: 'Regional Field Office - VI',
    label: 'DA-RFO-VI'
  },
  {
    value: 11,
    name: 'Regional Field Office - VII',
    label: 'DA-RFO-VII'
  },
  {
    value: 12,
    name: 'Regional Field Office - VIII',
    label: 'DA-RFO-VIII'
  },
  {
    value: 13,
    name: 'Regional Field Office - X',
    label: 'DA-RFO-X'
  },
  {
    value: 14,
    name: 'Regional Field Office - XI',
    label: 'DA-RFO-XI'
  },
  {
    value: 15,
    name: 'Regional Field Office - XII',
    label: 'DA-RFO-XII'
  },
  {
    value: 16,
    name: 'Regional Field Office - XIII',
    label: 'DA-RFO-XIII'
  },
  {
    value: 17,
    name: 'DAF-ARMM',
    label: 'DAF-ARMM'
  },
  {
    value: 18,
    name: 'Agricultural Training Institute',
    label: 'ATI'
  },
  {
    value: 19,
    name: 'Bureau of Animal Industry',
    label: 'BAI'
  },
  {
    value: 20,
    name: 'Bureau of Agricultural Research',
    label: 'BAR'
  },
  {
    value: 21,
    name: 'Bureau of Plant Industry',
    label: 'BPI'
  },
  {
    value: 22,
    name: 'Bureau of Soils and Water Management',
    label: 'BSWM'
  },
  {
    value: 23,
    name: 'Bureau of Agricultural and Fisheries Engineering',
    label: 'BAFE'
  },
  {
    value: 24,
    name: 'Agricultural Credit Policy Council ',
    label: 'ACPC'
  },
  {
    value: 25,
    name: 'BFAR Central Office',
    label: 'BFAR'
  },
  {
    value: 26,
    name: 'Fertilizer and Pesticvaluee Authority ',
    label: 'FPA'
  },
  {
    value: 27,
    name: 'National Fisheries Research and Development Institute ',
    label: 'NFRDI'
  },
  {
    value: 28,
    name: 'National Meat Inspection Service ',
    label: 'NMIS'
  },
  {
    value: 29,
    name: 'Philippine Rubber Research Institute',
    label: 'PRRI'
  },
  {
    value: 30,
    name: 'Philippine Carabao Center ',
    label: 'PCC'
  },
  {
    value: 31,
    name: 'Philippine Center for Post-Harvest Development and Mechanization',
    label: 'PHilMech'
  },
  {
    value: 32,
    name: 'Philippine Council for Agriculture and Fisheries',
    label: 'PCAF'
  },
  {
    value: 33,
    name: 'Philippine Fiber Industry Development Authority ',
    label: 'PhilFIDA'
  },
  {
    value: 34,
    name: 'National Dairy Authority',
    label: 'NDA'
  },
  {
    value: 35,
    name: 'National Food Authority',
    label: 'NFA'
  },
  {
    value: 36,
    name: 'National Irrigation Administration',
    label: 'NIA'
  },
  {
    value: 37,
    name: 'National Tobacco Administration',
    label: 'NTA'
  },
  {
    value: 38,
    name: 'Philippine Coconut Authority',
    label: 'PCA'
  },
  {
    value: 39,
    name: 'Philippine Crop Insurance Corporation',
    label: 'PCIC'
  },
  {
    value: 40,
    name: 'Philippine Fisheries Development Authority',
    label: 'PFDA'
  },
  {
    value: 41,
    name: 'Philippine Rice Research Institute',
    label: 'PhilRice'
  },
  {
    value: 42,
    name: 'Sugar Regulatory Administration',
    label: 'SRA'
  }
];

export const SPATIAL_COVERAGES = [
  {
    label: 'Nationwide',
    value: 1
  },
  {
    label: 'Inter-regional',
    value: 2
  },
  {
    label: 'Region-Specific',
    value: 3
  },
  {
    label: 'Abroad',
    value: 4
  }
];

export const YEARS = [
  {
    label: '2000',
    value: '2000'
  },
  {
    label: '2001',
    value: '2001'
  },
  {
    label: '2002',
    value: '2002'
  },
  {
    label: '2003',
    value: '2003'
  },
  {
    label: '2004',
    value: '2004'
  },
  {
    label: '2005',
    value: '2005'
  },
  {
    label: '2006',
    value: '2006'
  },
  {
    label: '2007',
    value: '2007'
  },
  {
    label: '2008',
    value: '2008'
  },
  {
    label: '2009',
    value: '2009'
  },
  {
    label: '2010',
    value: '2010'
  },
  {
    label: '2011',
    value: '2011'
  },
  {
    label: '2012',
    value: '2012'
  },
  {
    label: '2013',
    value: '2013'
  },
  {
    label: '2014',
    value: '2014'
  },
  {
    label: '2015',
    value: '2015'
  },
  {
    label: '2016',
    value: '2016'
  },
  {
    label: '2017',
    value: '2017'
  },
  {
    label: '2018',
    value: '2018'
  },
  {
    label: '2019',
    value: '2019'
  },
  {
    label: '2020',
    value: '2020'
  },
  {
    label: '2021',
    value: '2021'
  },
  {
    label: '2022',
    value: '2022'
  },
  {
    label: '2023',
    value: '2023'
  },
  {
    label: '2024',
    value: '2024'
  },
  {
    label: '2025',
    value: '2025'
  },
  {
    label: '2026',
    value: '2026'
  },
  {
    label: '2027',
    value: '2027'
  },
  {
    label: '2028',
    value: '2028'
  },
  {
    label: '2029',
    value: '2029'
  },
  {
    label: '2030',
    value: '2030'
  }
];

export const TECHNICAL_READINESSES = [
  {
    value: 1,
    label: 'Concept Note'
  },
  {
    value: 2,
    label: 'Project Proposal'
  },
  {
    value: 3,
    label: 'Detailed Engineering Design'
  },
  {
    value: 4,
    label: 'Business Plan'
  },
  {
    value: 5,
    label: 'Feasibility Studies'
  },
  {
    value: 6,
    label: 'Right of Way'
  },
  {
    value: 7,
    label: 'Resettlement Action Plan'
  }
];

export const ROLES = [
  {
    value: 1,
    label: 'superadmin'
  },
  {
    value: 2,
    label: 'admin'
  },
  {
    value: 3,
    label: 'ipd'
  },
  {
    value: 4,
    label: 'banner programs'
  },
  {
    value: 5,
    label: 'spcmad'
  },
  {
    value: 6,
    label: 'pds'
  },
  {
    value: 7,
    label: 'supervisor'
  },
  {
    value: 8,
    label: 'encoder'
  }
];

export const DISTRICTS = [
  { value: 1, province_id: 4, label: 'Abra, Lone' },
  { value: 2, province_id: 72, label: 'Agusan del Norte, 1st' },
  { value: 3, province_id: 72, label: 'Agusan del Norte, 2nd' },
  { value: 4, province_id: 73, label: 'Agusan del Sur, 1st' },
  { value: 5, province_id: 73, label: 'Agusan del Sur, 2nd' },
  { value: 6, province_id: 40, label: 'Aklan, 1st' },
  { value: 7, province_id: 40, label: 'Aklan, 2nd' },
  { value: 8, province_id: 36, label: 'Albay, 1st' },
  { value: 9, province_id: 36, label: 'Albay, 2nd' },
  { value: 10, province_id: 36, label: 'Albay, 3rd' },
  { value: 11, province_id: 23, label: 'Antipolo, 1st' },
  { value: 12, province_id: 23, label: 'Antipolo, 2nd' },
  { value: 13, province_id: 41, label: 'Antique, Lone' },
  { value: 14, province_id: 5, label: 'Apayao, Lone' },
  { value: 15, province_id: 22, label: 'Aurora, Lone' },
  { value: 16, province_id: 45, label: 'Bacolod, Lone' },
  { value: 17, province_id: 3, label: 'Baguio, Lone' },
  { value: 18, province_id: 78, label: 'Basilan, Lone' },
  { value: 19, province_id: 16, label: 'Bataan, 1st' },
  { value: 20, province_id: 16, label: 'Bataan, 2nd' },
  { value: 21, province_id: 15, label: 'Batanes, Lone' },
  { value: 22, province_id: 26, label: 'Batangas, 1st' },
  { value: 23, province_id: 26, label: 'Batangas, 2nd' },
  { value: 24, province_id: 26, label: 'Batangas, 3rd' },
  { value: 25, province_id: 26, label: 'Batangas, 4th' },
  { value: 26, province_id: 26, label: 'Batangas, 5th' },
  { value: 27, province_id: 26, label: 'Batangas, 6th' },
  { value: 28, province_id: 3, label: 'Benguet, Lone' },
  { value: 29, province_id: 54, label: 'Biliran, Lone' },
  { value: 30, province_id: 25, label: 'Biñan, Lone' },
  { value: 31, province_id: 47, label: 'Bohol, 1st' },
  { value: 32, province_id: 47, label: 'Bohol, 2nd' },
  { value: 33, province_id: 47, label: 'Bohol, 3rd' },
  { value: 34, province_id: 59, label: 'Bukidnon, 1st' },
  { value: 35, province_id: 59, label: 'Bukidnon, 2nd' },
  { value: 36, province_id: 59, label: 'Bukidnon, 3rd' },
  { value: 37, province_id: 59, label: 'Bukidnon, 4th' },
  { value: 38, province_id: 20, label: 'Bulacan, 1st' },
  { value: 39, province_id: 20, label: 'Bulacan, 2nd' },
  { value: 40, province_id: 20, label: 'Bulacan, 3rd' },
  { value: 41, province_id: 20, label: 'Bulacan, 4th' },
  { value: 42, province_id: 12, label: 'Cagayan, 1st' },
  { value: 43, province_id: 12, label: 'Cagayan, 2nd' },
  { value: 44, province_id: 12, label: 'Cagayan, 3rd' },
  { value: 45, province_id: 93, label: 'Cagayan de Oro, 1st' },
  { value: 46, province_id: 93, label: 'Cagayan de Oro, 2nd' },
  { value: 47, province_id: 25, label: 'Calamba, Lone' },
  { value: 48, province_id: 84, label: 'Caloocan, 1st' },
  { value: 49, province_id: 84, label: 'Caloocan, 2nd' },
  { value: 50, province_id: 34, label: 'Camarines Norte, 1st' },
  { value: 51, province_id: 34, label: 'Camarines Norte, 2nd' },
  { value: 52, province_id: 38, label: 'Camarines Sur, 1st' },
  { value: 53, province_id: 38, label: 'Camarines Sur, 2nd' },
  { value: 54, province_id: 38, label: 'Camarines Sur, 3rd' },
  { value: 55, province_id: 38, label: 'Camarines Sur, 4th' },
  { value: 56, province_id: 38, label: 'Camarines Sur, 5th' },
  { value: 57, province_id: 62, label: 'Camiguin, Lone' },
  { value: 58, province_id: 39, label: 'Capiz, 1st' },
  { value: 59, province_id: 39, label: 'Capiz, 2nd' },
  { value: 60, province_id: 33, label: 'Catanduanes, Lone' },
  { value: 61, province_id: 24, label: 'Cavite, 1st' },
  { value: 62, province_id: 24, label: 'Cavite, 2nd' },
  { value: 63, province_id: 24, label: 'Cavite, 3rd' },
  { value: 64, province_id: 24, label: 'Cavite, 4th' },
  { value: 65, province_id: 24, label: 'Cavite, 5th' },
  { value: 66, province_id: 24, label: 'Cavite, 6th' },
  { value: 67, province_id: 24, label: 'Cavite, 7th' },
  { value: 68, province_id: 24, label: 'Cavite, 8th' },
  { value: 69, province_id: 46, label: 'Cebu, 1st' },
  { value: 70, province_id: 46, label: 'Cebu, 2nd' },
  { value: 71, province_id: 46, label: 'Cebu, 3rd' },
  { value: 72, province_id: 46, label: 'Cebu, 4th' },
  { value: 73, province_id: 46, label: 'Cebu, 5th' },
  { value: 74, province_id: 46, label: 'Cebu, 6th' },
  { value: 75, province_id: 46, label: 'Cebu, 7th' },
  { value: 76, province_id: 46, label: 'Cebu City, 1st' },
  { value: 77, province_id: 46, label: 'Cebu City, 2nd' },
  { value: 78, province_id: 64, label: 'Compostela Valley, 1st' },
  { value: 79, province_id: 64, label: 'Compostela Valley, 2nd' },
  { value: 80, province_id: 70, label: 'Cotabato, 1st' },
  { value: 81, province_id: 70, label: 'Cotabato, 2nd' },
  { value: 82, province_id: 70, label: 'Cotabato, 3rd' },
  { value: 83, province_id: 87, label: 'Davao City, 1st' },
  { value: 84, province_id: 87, label: 'Davao City, 2nd' },
  { value: 85, province_id: 87, label: 'Davao City, 3rd' },
  { value: 86, province_id: 67, label: 'Davao del Norte, 1st' },
  { value: 87, province_id: 67, label: 'Davao del Norte, 2nd' },
  { value: 88, province_id: 65, label: 'Davao del Sur, Lone' },
  { value: 89, province_id: 66, label: 'Davao Occidental, Lone' },
  { value: 90, province_id: 63, label: 'Davao Oriental, 1st' },
  { value: 91, province_id: 63, label: 'Davao Oriental, 2nd' },
  { value: 92, province_id: 76, label: 'Dinagat Islands, Lone' },
  { value: 93, province_id: 50, label: 'Eastern Samar, Lone' },
  { value: 94, province_id: 44, label: 'Guimaras, Lone' },
  { value: 95, province_id: 2, label: 'Ifugao, Lone' },
  { value: 96, province_id: 92, label: 'Iligan, Lone' },
  { value: 97, province_id: 8, label: 'Ilocos Norte, 1st' },
  { value: 98, province_id: 8, label: 'Ilocos Norte, 2nd' },
  { value: 99, province_id: 9, label: 'Ilocos Sur, 1st' },
  { value: 100, province_id: 9, label: 'Ilocos Sur, 2nd' },
  { value: 101, province_id: 43, label: 'Iloilo, 1st' },
  { value: 102, province_id: 43, label: 'Iloilo, 2nd' },
  { value: 103, province_id: 43, label: 'Iloilo, 3rd' },
  { value: 104, province_id: 43, label: 'Iloilo, 4th' },
  { value: 105, province_id: 43, label: 'Iloilo, 5th' },
  { value: 106, province_id: 88, label: 'Iloilo City, Lone' },
  { value: 107, province_id: 13, label: 'Isabela, 1st' },
  { value: 108, province_id: 13, label: 'Isabela, 2nd' },
  { value: 109, province_id: 13, label: 'Isabela, 3rd' },
  { value: 110, province_id: 13, label: 'Isabela, 4th' },
  { value: 111, province_id: 13, label: 'Isabela, 5th' },
  { value: 112, province_id: 13, label: 'Isabela, 6th' },
  { value: 113, province_id: 6, label: 'Kalinga, Lone' },
  { value: 114, province_id: 7, label: 'La Union, 1st' },
  { value: 115, province_id: 7, label: 'La Union, 2nd' },
  { value: 116, province_id: 25, label: 'Laguna, 1st' },
  { value: 117, province_id: 25, label: 'Laguna, 2nd' },
  { value: 118, province_id: 25, label: 'Laguna, 3rd' },
  { value: 119, province_id: 25, label: 'Laguna, 4th' },
  { value: 120, province_id: 60, label: 'Lanao del Norte, 1st' },
  { value: 121, province_id: 60, label: 'Lanao del Norte, 2nd' },
  { value: 122, province_id: 81, label: 'Lanao del Sur, 1st' },
  { value: 123, province_id: 81, label: 'Lanao del Sur, 2nd' },
  { value: 124, province_id: 46, label: 'Lapu-Lapu City, Lone' },
  { value: 125, province_id: 85, label: 'Las Piñas, Lone' },
  { value: 126, province_id: 53, label: 'Leyte, 1st' },
  { value: 127, province_id: 53, label: 'Leyte, 2nd' },
  { value: 128, province_id: 53, label: 'Leyte, 3rd' },
  { value: 129, province_id: 53, label: 'Leyte, 4th' },
  { value: 130, province_id: 53, label: 'Leyte, 5th' },
  { value: 131, province_id: 80, label: 'Maguindanao, 1st' },
  { value: 132, province_id: 80, label: 'Maguindanao, 2nd' },
  { value: 133, province_id: 85, label: 'Makati, 1st' },
  { value: 134, province_id: 85, label: 'Makati, 2nd' },
  { value: 135, province_id: 84, label: 'Malabon, Lone' },
  { value: 136, province_id: 83, label: 'Mandaluyong, Lone' },
  { value: 137, province_id: 82, label: 'Manila, 1st' },
  { value: 138, province_id: 82, label: 'Manila, 2nd' },
  { value: 139, province_id: 82, label: 'Manila, 3rd' },
  { value: 140, province_id: 82, label: 'Manila, 4th' },
  { value: 141, province_id: 82, label: 'Manila, 5th' },
  { value: 142, province_id: 82, label: 'Manila, 6th' },
  { value: 143, province_id: 82, label: 'Marikina, 1st' },
  { value: 144, province_id: 82, label: 'Marikina, 2nd' },
  { value: 145, province_id: 32, label: 'Marinduque, Lone' },
  { value: 146, province_id: 37, label: 'Masbate, 1st' },
  { value: 147, province_id: 37, label: 'Masbate, 2nd' },
  { value: 148, province_id: 37, label: 'Masbate, 3rd' },
  { value: 149, province_id: 58, label: 'Misamis Occidental, 1st' },
  { value: 150, province_id: 58, label: 'Misamis Occidental, 2nd' },
  { value: 151, province_id: 61, label: 'Misamis Oriental, 1st' },
  { value: 152, province_id: 61, label: 'Misamis Oriental, 2nd' },
  { value: 153, province_id: 1, label: 'Mountain Province, Lone' },
  { value: 154, province_id: 85, label: 'Muntinlupa, Lone' },
  { value: 155, province_id: 84, label: 'Navotas, Lone' },
  { value: 156, province_id: 42, label: 'Negros Occidental, 1st' },
  { value: 157, province_id: 42, label: 'Negros Occidental, 2nd' },
  { value: 158, province_id: 42, label: 'Negros Occidental, 3rd' },
  { value: 159, province_id: 42, label: 'Negros Occidental, 4th' },
  { value: 160, province_id: 42, label: 'Negros Occidental, 5th' },
  { value: 161, province_id: 42, label: 'Negros Occidental, 6th' },
  { value: 162, province_id: 45, label: 'Negros Oriental, 1st' },
  { value: 163, province_id: 45, label: 'Negros Oriental, 2nd' },
  { value: 164, province_id: 45, label: 'Negros Oriental, 3rd' },
  { value: 165, province_id: 51, label: 'Northern Samar, 1st' },
  { value: 166, province_id: 51, label: 'Northern Samar, 2nd' },
  { value: 167, province_id: 21, label: 'Nueva Ecija, 1st' },
  { value: 168, province_id: 21, label: 'Nueva Ecija, 2nd' },
  { value: 169, province_id: 21, label: 'Nueva Ecija, 3rd' },
  { value: 170, province_id: 21, label: 'Nueva Ecija, 4th' },
  { value: 171, province_id: 11, label: 'Nueva Vizcaya, Lone' },
  { value: 172, province_id: 28, label: 'Occidental Mindoro, Lone' },
  { value: 173, province_id: 29, label: 'Oriental Mindoro, 1st' },
  { value: 174, province_id: 29, label: 'Oriental Mindoro, 2nd' },
  { value: 175, province_id: 31, label: 'Palawan, 1st' },
  { value: 176, province_id: 31, label: 'Palawan, 2nd' },
  { value: 177, province_id: 31, label: 'Palawan, 3rd' },
  { value: 178, province_id: 19, label: 'Pampanga, 1st' },
  { value: 179, province_id: 19, label: 'Pampanga, 2nd' },
  { value: 180, province_id: 19, label: 'Pampanga, 3rd' },
  { value: 181, province_id: 19, label: 'Pampanga, 4th' },
  { value: 182, province_id: 10, label: 'Pangasinan, 1st' },
  { value: 183, province_id: 10, label: 'Pangasinan, 2nd' },
  { value: 184, province_id: 10, label: 'Pangasinan, 3rd' },
  { value: 185, province_id: 10, label: 'Pangasinan, 4th' },
  { value: 186, province_id: 10, label: 'Pangasinan, 5th' },
  { value: 187, province_id: 10, label: 'Pangasinan, 6th' },
  { value: 188, province_id: 85, label: 'Parañaque, 1st' },
  { value: 189, province_id: 85, label: 'Parañaque, 2nd' },
  { value: 190, province_id: 83, label: 'Pasay, Lone' },
  { value: 191, province_id: 85, label: 'Pasig, Lone' },
  { value: 192, province_id: 27, label: 'Quezon, 1st' },
  { value: 193, province_id: 27, label: 'Quezon, 2nd' },
  { value: 194, province_id: 27, label: 'Quezon, 3rd' },
  { value: 195, province_id: 27, label: 'Quezon, 4th' },
  { value: 196, province_id: 83, label: 'Quezon City, 1st' },
  { value: 197, province_id: 83, label: 'Quezon City, 2nd' },
  { value: 198, province_id: 83, label: 'Quezon City, 3rd' },
  { value: 199, province_id: 83, label: 'Quezon City, 4th' },
  { value: 200, province_id: 83, label: 'Quezon City, 5th' },
  { value: 201, province_id: 83, label: 'Quezon City, 6th' },
  { value: 202, province_id: 14, label: 'Quirino, Lone' },
  { value: 203, province_id: 23, label: 'Rizal, 1st' },
  { value: 204, province_id: 23, label: 'Rizal, 2nd' },
  { value: 205, province_id: 30, label: 'Romblon, Lone' },
  { value: 206, province_id: 52, label: 'Samar, 1st' },
  { value: 207, province_id: 52, label: 'Samar, 2nd' },
  { value: 208, province_id: 20, label: 'San Jose del Monte, Lone' },
  { value: 209, province_id: 83, label: 'San Juan, Lone' },
  { value: 210, province_id: 71, label: 'Sarangani, Lone' },
  { value: 211, province_id: 48, label: 'Siquijor, Lone' },
  { value: 212, province_id: 35, label: 'Sorsogon, 1st' },
  { value: 213, province_id: 35, label: 'Sorsogon, 2nd' },
  { value: 214, province_id: 68, label: 'South Cotabato, 1st' },
  { value: 215, province_id: 68, label: 'South Cotabato, 2nd' },
  { value: 216, province_id: 49, label: 'Southern Leyte, Lone' },
  { value: 217, province_id: 69, label: 'Sultan Kudarat, 1st' },
  { value: 218, province_id: 69, label: 'Sultan Kudarat, 2nd' },
  { value: 219, province_id: 79, label: 'Sulu, 1st' },
  { value: 220, province_id: 79, label: 'Sulu, 2nd' },
  { value: 221, province_id: 75, label: 'Surigao del Norte, 1st' },
  { value: 222, province_id: 75, label: 'Surigao del Norte, 2nd' },
  { value: 223, province_id: 74, label: 'Surigao del Sur, 1st' },
  { value: 224, province_id: 74, label: 'Surigao del Sur, 2nd' },
  { value: 225, province_id: 85, label: 'Taguig–Pateros, 1st' },
  { value: 226, province_id: 85, label: 'Taguig, 2nd' },
  { value: 227, province_id: 18, label: 'Tarlac, 1st' },
  { value: 228, province_id: 18, label: 'Tarlac, 2nd' },
  { value: 229, province_id: 18, label: 'Tarlac, 3rd' },
  { value: 230, province_id: 77, label: 'Tawi-Tawi, Lone' },
  { value: 231, province_id: 84, label: 'Valenzuela, 1st' },
  { value: 232, province_id: 84, label: 'Valenzuela, 2nd' },
  { value: 233, province_id: 17, label: 'Zambales, 1st' },
  { value: 234, province_id: 17, label: 'Zambales, 2nd' },
  { value: 235, province_id: 86, label: 'Zamboanga City, 1st' },
  { value: 236, province_id: 86, label: 'Zamboanga City, 2nd' },
  { value: 237, province_id: 56, label: 'Zamboanga del Norte, 1st' },
  { value: 238, province_id: 56, label: 'Zamboanga del Norte, 2nd' },
  { value: 239, province_id: 56, label: 'Zamboanga del Norte, 3rd' },
  { value: 240, province_id: 57, label: 'Zamboanga del Sur, 1st' },
  { value: 241, province_id: 57, label: 'Zamboanga del Sur, 2nd' },
  { value: 242, province_id: 55, label: 'Zamboanga Sibugay, 1st' },
  { value: 243, province_id: 55, label: 'Zamboanga Sibugay, 2nd' }
];
