<template>
  <q-page class="q-pt-lg">
    <page-title title="GAD Responsiveness"></page-title>
    <q-list>
      <q-item-label header class="text-right text-weight-bold">Total Score: {{ totalScore }} ({{ totalScore | interpretation }})</q-item-label>
      <q-expansion-item
        expand-separator
        :label="key"
      ...
      <q-btn label="Calculate" @click="calculateScore"></q-btn>
    </q-list>
  </q-page>
</template>
...
export default {
  ...
  filters: {
    interpretation(val) {
      ...
      else if (val >= 4 && val <= 7.9) return 'Proposed project has promising prospects'
      else if (val >= 8 && val <= 14.9) return 'Proposed project is gender-sensitive'
